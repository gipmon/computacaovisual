...
int main( int argc, char** argv )
{
    ...
    //createHistogramImage(Mat histograma)
    ...
    double minGrayValue, maxGrayValue;
    minMaxLoc(imagem, &minGrayValue, &maxGrayValue);
    double factor = 255.0 / (maxGrayValue - minGrayValue);

    Mat imagemCS = Mat (imagem.size(), CV_8UC1);

    for( int row = 0; row < imagem.size().height; ++row){
        for( int col=0; col < imagemCS.size().width; ++col){
            imagemCS.at<unsigned char>(row, col) = (int) ((imagem.at<unsigned char>(row, col) - minGrayValue) * factor);
        }
    }
